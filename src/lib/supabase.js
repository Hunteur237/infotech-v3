...
export const productsService={getAll:async()=>{const{data,error}=await supabase.from('products').select('*').eq('active',true).order('created_at',{ascending:false});if(error)throw error;return data},getAllAdmin:()=>q('products').getAll({order:'created_at'}),insert:d=>q('products').insert(d),update:(id,d)=>q('products').update(id,d),delete:(id)=>q('products').update(id,{active:false})}
