...
export default function RDVModal({ open, onClose }) {
  ...
  const confirm = async () => {
    ...
    setLoading(true)
    try { await appointmentsService.insert({ name: form.name, phone: form.phone, subject: form.subject, day, slot, user_id: user?.id || null }); notify('rdv', { name: form.name, phone: form.phone, subject: form.subject, day, slot, email: form.email }) }
